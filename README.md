...
```bash
  git clone -b testnet --depth 1 https://github.com/blockstack/stacks-local-dev ./stacks-local-dev && cd ./stacks-local-dev
```
...
The first container to start will always be the `testnet_bootstrap` container. The sole purpose of this container is to run a [script](https://github.com/blockstack/stacks-local-dev/blob/testnet/setup.sh) to replace the variables in the `.template` files with the values from `.env`

## API Container

The API Container will run a [script](https://github.com/blockstack/stacks-local-dev/blob/testnet/setup-bns.sh) before starting it's server. The sole purpose of this is to download (or verify the files exist) V1 BNS data. Once the download/extraction/verification has finished, the `stacks-blockchain-api` server will start up

## Env Vars

All variables used in the [`sample.env`](https://github.com/blockstack/stacks-local-dev/blob/testnet/sample.env) file can be modified, but generally most of them should be left as-is.
...
In this section of the [`sample.env`](https://github.com/blockstack/stacks-local-dev/blob/testnet/sample.env) file, the values can be modified to change the ports opened locally by docker.
...
This password is defined in the file [`sample.env`](https://github.com/blockstack/stacks-local-dev/blob/testnet/sample.env#L59) 
...
### Services Running in Testnet
**docker-compose Testnet service names**:
- follower
- api
- postgres
...
- To resolve, find the port you have in use (i.e. `5432` and edit the [`sample.env`](https://github.com/blockstack/stacks-local-dev/blob/testnet/sample.env) file to use the new port)
